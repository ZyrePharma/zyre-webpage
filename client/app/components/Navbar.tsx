    ...
    <>
      ...
      >
        <div className="flex justify-between items-center h-[100px] sm:h-[110px] md:h-[120px] px-4 sm:px-6 md:px-8 lg:px-12 relative">
          ...
          <button
            onClick={toggleMenu}
            className={`lg:hidden ${isOpen ? 'fixed top-8 right-4 sm:right-6 md:right-8 z-[100]' : 'relative z-[80]'}`}
          >
            <motion.div
      ...
      {/* Mobile Sidebar Menu and Backdrop */}
      <AnimatePresence>
        {isOpen && (
          <>
            {/* Backdrop - Now outside navbar */}
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            <motion.div
              ...
              className="fixed top-0 right-0 w-[85%] sm:w-[70%] md:w-[60%] h-screen bg-white/95 backdrop-blur-md shadow-2xl flex flex-col items-center py-20 z-[70] border-l border-gray-200 lg:hidden"
            >
